...
export default function Dashboard() {
  ...
  const submitChanges = async (e) => {
    ...
    if (email === "" && oldPwd !== "" && newPwd !== "") {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/authentication/accountsettings`,
        {
          method: "PUT",
    ...
    if (oldPwd !== "" && newPwd !== 0 && email !== "") {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/authentication/accountsettings`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ newEmail: email, oldPwd, newPwd }),
          credentials: "include",
        }
      );
      const json = await res.json();
  ...
  function PostItem({ _id, title, author, createdAt, userInfo }) {
    const deletePost = async (e) => {
      ...
      if (res.ok) {
        const res2 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts`);
